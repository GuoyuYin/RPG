...
func RunManager(cfg *mgrconfig.Config) {
	...
	mgr.target.DF_staticPriosPath = cfg.DF_PrioTable
...
func (mgr *Manager) heartbeadCorpu() {
	lastTime := time.Now()

	corpus := mgr.corpus

	// conbine mgrconfig.workdir and corpus_data.gob
	corpu_path := filepath.Join(mgr.cfg.Workdir, "corpus_data.gob")
	// Open or create a file to save the binary data
	file, err := os.OpenFile(corpu_path, os.O_CREATE|os.O_WRONLY|os.O_APPEND, 0644)
	if err != nil {
	...
	// Create a new ticker that ticks every 300 seconds (5 minutes)
	for now := range time.NewTicker(30 * time.Second).C {
		_ = int(now.Sub(lastTime)) // Just to update the time difference, you might want to use it
		lastTime = now
		...
		CorpusInfo := corpus.SaveCorpus()

		// For each CorpusInfo, write it to the file in binary format
		encoder := gob.NewEncoder(file)
		for _, info := range CorpusInfo {
			if err := encoder.Encode(info); err != nil {
				fmt.Println("Error encoding to file:", err)
			}
		}
		log.Logf(0, "Corpus data saved")
	}
}
