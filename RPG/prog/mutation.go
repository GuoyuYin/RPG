...
func (p *Prog) df_CheckForConfigCall() bool {
	for _, c := range p.Calls {
		if strings.HasPrefix(c.Meta.Name, "syz_proc") || strings.HasPrefix(c.Meta.Name, "syz_sys") {
			return true
		}
	}
	return false
...
func (p *Prog) Mutate(rs rand.Source, ncalls int, ct *ChoiceTable, noMutate map[int]bool, corpus []*Prog) {
	if p.df_CheckForConfigCall() {
		p.MutateWithOpts(rs, ncalls, ct, noMutate, corpus, DefaultMutateOpts)
	} else {
		// df: adding configcall here
		p.df_MutateWithOpts(rs, ncalls, ct, noMutate, corpus, DefaultMutateOpts)
		for _, c := range p.Calls {
			log.Logf(1, "df: call in mutation %v", c.Meta.Name)
		}
	}
...
func (p *Prog) df_MutateWithOpts(rs rand.Source, ncalls int, ct *ChoiceTable, noMutate map[int]bool,
...
// func (ctx *mutator) df_insertConfig() bool {
// 	log.Logf(1, "df: insertConfig in mutation")
// 	p, r := ctx.p, ctx.r
// 	if len(p.Calls) >= ctx.ncalls {
// 		return false
// 	}
// 	configCalls := []*Call{}
// 	configResetCalls := []*Call{}
// 	for _, c := range p.Calls {
// 		if r.randInt(2) == 0 {
// 			s := analyze(ctx.ct, ctx.corpus, p, c)
// 			configCall := r.df_generateConfigSet(s, p, c.Meta.ID)
// 			log.Logf(1, "df: Adding set call in mutation %v", configCall[0].Meta.Name)
// 			configResetCall := r.df_generateConfigReset(s, p, configCall[0].Meta.ID)
// 			configCalls = append(configCalls, configCall[0])
// 			configResetCalls = append(configResetCalls, configResetCall[0])
// 			log.Logf(1, "df: Adding reset call in mutation %v", configResetCall[0].Meta.Name)
// 		}
// 	}

// 	totalCalls := len(p.Calls) + len(configCalls) + len(configResetCalls)
// 	if totalCalls > ctx.ncalls {
// 		// Remove from the end until totalCalls <= ctx.ncalls
// 		// (We assume configCalls and configResetCalls are paired in order)
// 		for totalCalls > ctx.ncalls && len(configCalls) > 0 {
// 			// Pop the last element of each
// 			configCalls = configCalls[:len(configCalls)-1]
// 			configResetCalls = configResetCalls[:len(configResetCalls)-1]
// 			totalCalls = len(p.Calls) + len(configCalls) + len(configResetCalls)
// 		}
// 	}

// 	newCalls := make([]*Call, 0, len(p.Calls)+len(configCalls)+len(configResetCalls))
// 	newCalls = append(newCalls, configCalls...)
// 	newCalls = append(newCalls, p.Calls...)
// 	newCalls = append(newCalls, configResetCalls...)
// 	p.Calls = newCalls
...
func (ctx *mutator) df_insertConfig() bool {
	...
	if idx < len(p.Calls) {
		...
		array_idx = p.Calls[0].Meta.ID
	}

	s := analyze(ctx.ct, ctx.corpus, p, c)
	config_calls := r.df_generateConfigSet(s, p, array_idx)
	// log.Logf(1, "df: Adding set call in mutation %v", config_calls[0].Meta.Name, config_calls[0].Meta.ID)
	config_reset_calls := r.df_generateConfigReset(s, p, config_calls[0].Meta.ID)
	// log.Logf(1, "df: Adding reset call in mutation %v", config_reset_calls[0].Meta.Name, config_reset_calls[0].Meta.ID)
	p.insertBefore(c, config_calls)
	p.Calls = append(p.Calls, config_reset_calls...)
