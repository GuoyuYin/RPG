...
func (r *randGen) df_generateConfigSet(s *state, p *Prog, syscall_idx int) []*Call {
	idx := s.ct.DF_choiceConfigcall(r.Rand, syscall_idx)
	meta := r.target.Syscalls[idx]
	return r.generateParticularCall(s, meta)
}

func (r *randGen) df_generateConfigReset(s *state, p *Prog, configcall_idx int) []*Call {
	idx := s.ct.DF_choiceRestConfigcall(r.Rand, configcall_idx)
	meta := r.target.Syscalls[idx]
	return r.generateParticularCall(s, meta)
}
...
func (r *randGen) generateParticularCall(s *state, meta *Syscall) (calls []*Call) {
	if meta.Attrs.Disabled {
		log.Logf(0, "generating disabled call %v", meta.ID)
