...
func (corpus *Corpus) SaveCorpus() []CorpusInfo {
	infos := new([]CorpusInfo)
	for _, item := range corpus.progs {
		CorpusInfo := new(CorpusInfo)
		CorpusInfo.Prog = item.Prog.SerializeVerbose()
		CorpusInfo.Cover = item.Cover
		log.Logf(0, "corpus: saving program %v", item.Prog)
		log.Logf(0, "corpus: saving signal %v", item.Cover)
		*infos = append(*infos, *CorpusInfo)
	}
	return *infos
}
